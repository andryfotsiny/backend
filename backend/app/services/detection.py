...
class DetectionService:
    ...
    async def check_phone(
        ...
        if cached:
            return {**cached, "response_time_ms": int((time.time() - start_time) * 1000)}
        ...
        if fraud_entry:
            ...
            await self._log_detection(db, user_id, "phone", True, fraud_entry.confidence_score, "blacklist", int((time.time() - start_time) * 1000))
            return response
        
        is_fraud, confidence = ml_service.predict_phone(phone, {"hour": 14, "call_count": 1})
        ...
        await self._log_detection(db, user_id, "phone", is_fraud, confidence, "ml", int((time.time() - start_time) * 1000))
    ...
    async def check_sms(
        ...
        await self._log_detection(db, user_id, "sms", is_fraud, confidence, "ml_rag", int((time.time() - start_time) * 1000))
    ...
    async def check_email(
        ...
        if fraud_domain:
            ...
            await self._log_detection(db, user_id, "email", True, fraud_domain.reputation_score, "blacklist", int((time.time() - start_time) * 1000))
            return response
        ...
        await self._log_detection(db, user_id, "email", is_fraud, confidence, "ml", int((time.time() - start_time) * 1000))
